...
def create_nerf(args):
    ...
    reset_optimizer = args.re_start_optimizer
...
def config_parser():
    ...
    parser.add_argument("--re_start_optimizer", type=int, default=0, 
                        help='reset optimizer at re-starting training')
...
def train():
    ...
    # Load data
    # 注意这里的factor是指缩放倍数，比如factor=8，就是缩放到1/8
    # 这里的scale_split是指每个stage的数据量，具体的值就可以到/data/posses_enu.json中查看，当cur_stage越大，图像集就越大
    ...
    print('Train set size:', images.shape[0])
    print('i_train set size:', i_train.shape[0])
    ...
    if args.render_test:
        ...
        with torch.no_grad():
            ...
            imageio.mimwrite(os.path.join(testsavedir, 'video.mp4'), to8b(rgbs), fps=24, quality=10)
            return
